...
if(isset($_GET['id']))
{
    try
    {
        $dato=$_GET['id'];$sql=$conn->query("select *from estudiantes where id like '%$dato%'");
...
<hr>
<input type="button"class="bt btn-success"onclick="location.href='datosAlumno.php'"name="Volver"value="Volver a la consulta de datos">
...
<h5 class="modal-title"id="exampleModalLabel">AgregarNuevoRegistro</h5>
...
<button type="submit"class="btn btn-primary">GuardarElRegistro</button>
</div>
</form>
</div>
...
<?php
 ...
 {
    try{
        if(isset($_POST['id'])){$sql=$conn->exec("UPDATE estudiantes SET nombres='".$_POST['nombres']."',apellidos='".$_POST['apellidos']."'WHERE id='".$_POST['id']."'");
